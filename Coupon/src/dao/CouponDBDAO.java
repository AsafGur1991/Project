...
public class CouponDBDAO implements CouponDAO, Serializable
{
	 ...
	 */
	@Override
	public void removeCoupon(Coupon coupon) throws SQLException, ClassNotFoundException, InterruptedException, NullConnectionException, WrongEntryException, UnAuthorizedAction
	{
		//establishing the connection to the data base
		Connection con = (Connection) ConnectionPool.getInstance().getConnection();
		...
		rs = stmt.executeQuery(String.format(CouponSqlQuerys.ALL_COUPONS_BY_ID, coupon.getId()));
		if (rs.next())
		{
			Statement testStmt = (Statement) con.createStatement();
			ResultSet testRs;
			//a mysql statement that checks if the customer exist in the database
			testRs = stmt.executeQuery(String.format(CouponSqlQuerys.COUPON_ID_BY_COMP_ID_AND_COUPON_ID, this.userCompanyId, coupon.getId()));
			if (testRs.next())
			{
				...
				//returning the connection
				ConnectionPool.getInstance().returnConnection(con);
				System.out.println("coupon has been removed by company "+ this.getUserCompanyId());
			}
			else
			{
				throw new UnAuthorizedAction("company " + this.getUserCompanyId() +"tried to remove a coupon that is not his");
			}
		}
		else
		{
			throw new WrongEntryException("company " + this.getUserCompanyId() + "tried to remove a coupon that does not exist in the datbase");
		}
	...
	public void updateCoupon(Coupon coupon) throws ClassNotFoundException, InterruptedException, SQLException, ParseException, NullConnectionException, WrongEntryException, UnAuthorizedAction
	{
		...
		if (rs.next())
		{
			...
			{
				throw new UnAuthorizedAction("company " + this.getUserCompanyId() +"tried to update a coupon that is not his");
			}
		}
		else
		{
			throw new WrongEntryException("company " + this.getUserCompanyId() + "tried to update a coupon that does not exist in the datbase");
		}
	}
	...
	public void threadRemoveCoupon(Coupon coupon) throws NullConnectionException, SQLException
	{
		//establishing the connection to the data base
		Connection con = (Connection) ConnectionPool.getInstance().getConnection();
		Statement stmt = (Statement) con.createStatement();
		ResultSet rs;
		//a mysql statement that checks if the coupon exist in the database
		rs = stmt.executeQuery(String.format(CouponSqlQuerys.ALL_COUPONS_BY_ID, coupon.getId()));
		if (rs.next())
		{
			// create the mysql delete statement for the coupon table
			...
			//returning the connection
			ConnectionPool.getInstance().returnConnection(con);
			System.out.println("coupon has been removed by thread");					
		}
	}
