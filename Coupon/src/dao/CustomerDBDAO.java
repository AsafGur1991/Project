...
public class CustomerDBDAO implements CustomerDAO, Serializable
{
	...
	public void removeCustomer(Customer customer) throws ClassNotFoundException, InterruptedException, SQLException, NullConnectionException, WrongEntryException
	{
		...
		{
			throw new WrongEntryException("the admin tried to remove a customer that does not exist in the datbase");
		}
	...
	public void updateCustomer(Customer customer) throws ClassNotFoundException, InterruptedException, SQLException, NullConnectionException, WrongEntryException
	{
		...
		ResultSet rs;
		//a mysql statement that checks if the customer exist in the database
		rs = stmt.executeQuery(CustomerSqlQuerys.SELECT_CUSTOMER_BY_ID + customer.getId() +"'");
		if (rs.next())
		{
		...
		{
			throw new WrongEntryException("the admin tried to update a customer that does not exist in the datbase");
		}
	}
	...
	public Customer getCustomer(long id) throws ClassNotFoundException, InterruptedException, SQLException, NullConnectionException, ParseException
	{
		...
		//returning the connection
		ConnectionPool.getInstance().returnConnection(con);
		this.setUserCustomerId(customer.getId());
		customer.setMycoupons(this.getCoupons());
	 ...
	 */
	public String purchaseCoupon(Coupon coupon) throws ClassNotFoundException, InterruptedException, SQLException, ParseException, DuplicateCouponTypeException, UnAvailableCouponException, NullConnectionException
	{
		...
		if (canPurchase==true)
		{
			...
			//returning the connection
			ConnectionPool.getInstance().returnConnection(con);
			return ("coupon has been purchased");
		}
		else
		{
			return ("cannot purchase coupon " + coupon.getTitle());
		}
	}
